...
??? quote "C-stateの拡張の話。一旦スルー"
    ...
    **Return Value Information**
    ...
    CSTオブジェクトリスト)、そのCステートの依存ドメイン番号、その $\mathrm{C}$ ステートの調整タイプ、および特定のCステートのドメインに属す論理プロセッサの数。特定のC状態が複数のドメインに属す可能性がある。すなわち、_CSDリストには、同じCStatelndex値を持つ複数のエントリが存在する可能性がある。
